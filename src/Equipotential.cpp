...
Equipotential::Equipotential(MoleculeTarget *moleculeTarget, GasBuffer *gas, unsigned int polarizability_flag, double temperature, double mu, double alpha, unsigned int gas_buffer_flag, FILE* logfile) {
...
fprintf(logfile,"*********************************************************\n");
fprintf(logfile,"Equipotential Ellipsoid: \n");
fprintf(logfile,"*********************************************************\n");
fprintf(logfile,"Axis Length: %f %f %f Ang\n", a, b, c);
fprintf(logfile,"Uncertainy: %f %f %f\n", aUncert, bUncert, cUncert);
fprintf(logfile,"Percent Uncert: %f %f %f\n", aUncert*100/a, bUncert*100/b, cUncert*100/b);
fprintf(logfile,"RMSD: %f\n", rmsd);
fprintf(logfile,"Covariance matrix: %f\n", rmsd);
fprintf(logfile,"{  %f  %f  %f  }\n", cov_aa, cov_ab, cov_ac);
fprintf(logfile,"{  %f  %f  %f  }\n", cov_ab, cov_bb, cov_bc);
fprintf(logfile,"{  %f  %f  %f  }\n", cov_ac, cov_bc, cov_cc);
fprintf(logfile,"Average ellipsoid axes: %f +/- %f Ang\n", avgAxes, avgAxesUncert);
fprintf(logfile,"Ellipsoid surface area: %f +/- %f Ang^2\n", surfArea, surfAreaUncert);

enlargeEllipsoidBoundry();

fprintf(logfile,"Ellipsoid axes further expanded by: %f Ang\n", enlargeAmount);
}
