...
System::System(string targetFilename, string logname, unsigned int nProbe, unsigned int nIter, unsigned int seed, unsigned int nthreads, double dt, double temperatureTarget, unsigned int gas_buffer_flag,
unsigned int equipotential_flag, double skin, unsigned int short_range_cutoff, double lj_cutoff, unsigned int long_range_flag, unsigned int long_range_cutoff, double coul_cutoff, 
unsigned int polarizability_flag, double alpha, string file_ff, unsigned int user_ff_flag, unsigned int force_type, double &ccs_file, double &ccs_err_file, double &mass_file, double &charge_file) {
// read the input
  double start = omp_get_wtime(); 
  this->targetFilename = targetFilename;
  this->logname = logname;
  this->nProbe = nProbe;                           // numbers of gas buffers 
  this->nIter = nIter;                             // numbers of CCS calculations
  this->seed = seed;                               // random number seed 
  this->nthreads = nthreads;                       // number of threads
  this->dt = dt;                                   // time step in fs 
  this->temperatureTarget = temperatureTarget;     // temperature in Kelvin
  this->gas_buffer_flag = gas_buffer_flag;         // He = 1, N2 = 2, CO2 = 3
  this->equipotential_flag = equipotential_flag; 
  this->skin = skin;                               // skin cell-size   
  this->short_range_cutoff = short_range_cutoff;   // yes = 1 and not = 0 for cut lennard-jones interacion
  this->lj_cutoff = lj_cutoff;                     // lennard-jones cutoff   
  this->long_range_flag = long_range_flag;         // apply coulomb interaction 
  this->long_range_cutoff = long_range_cutoff;     // yes = 1 and not = 0 for cut coulomb interacion
  this->coul_cutoff = coul_cutoff;                 // coulomb cutoff
  this->polarizability_flag = polarizability_flag; // apply induced-dipole interaction
  this->alpha = alpha;                             // polarizability
  this->file_ff = file_ff;                         // name of force field
  this->user_ff_flag = user_ff_flag;               // user force field flag
  ...
  moleculeTarget = new MoleculeTarget(targetFilename, gas_buffer_flag, file_ff, user_ff_flag, force_type, logfile); 
  double end_molecule = omp_get_wtime();
  fprintf(logfile,"orientation time of molecule target: %f s\n", end_molecule - start_molecule);

// reduced mass
if (gas_buffer_flag == 1) {
  mu = moleculeTarget->mass * gas->mass / (moleculeTarget->mass + gas->mass);
  fprintf(logfile,"target mass: %f amu\n", moleculeTarget->mass);
  fprintf(logfile,"gas mass: %f amu\n", gas->mass);
  fprintf(logfile,"reduce mass: %f amu\n", mu);
  fprintf(logfile,"charge state: %f e\n", moleculeTarget->Q);
...
mass_file = moleculeTarget->mass;
charge_file = moleculeTarget->Q;
...
if (equipotential_flag == 1) {
  equipotential = new Equipotential(moleculeTarget, gas, polarizability_flag, temperatureTarget, mu, alpha, gas_buffer_flag, logfile);
  a = equipotential->a;
  b = equipotential->b;
  c = equipotential->c;
  fprintf(logfile,"ellipsoid axis length: %f %f %f Ang\n", a, b, c);
} else geometric_ellipsoid();
  
double end_ellipsoid = omp_get_wtime();
fprintf(logfile,"ellipsoid calculation time: %f s\n",end_ellipsoid - start_ellipsoid);

bmax = max(max(a,b),c); // maximal impact parameter
fprintf(logfile,"maximal impact parameter: %f Ang\n", bmax);

// create the linked cell list
double start_linked_cell = omp_get_wtime();
linkedcell = new LinkedCell(moleculeTarget, a, b, c, lj_cutoff, skin, long_range_flag, long_range_cutoff, coul_cutoff, gas_buffer_flag, logfile);
double end_linked_cell = omp_get_wtime();
fprintf(logfile,"linked-cell calculation time: %f s\n", end_linked_cell - start_linked_cell);
...
fprintf(logfile,"*********************************************************\n");
fprintf(logfile,"Trajectory calculations \n");
fprintf(logfile,"*********************************************************\n");
...
for (int i = 0; i < Niter; i++) {
  ...
  fprintf(logfile,"Ntraj: %i\n", Ntraj);
  fprintf(logfile,"Nfree: %i\n", Nfree);
  fprintf(logfile,"Nscatter: %i\n", Nscatter);
  fprintf(logfile,"Nlost: %i\n", Nlost);
  fprintf(logfile,"omega: %f\n", 1.0/(float(Nscatter + Nfree))*dOmega);
}
  
double end_ccs = omp_get_wtime();
fprintf(logfile,"CCS time: %f s\n",end_ccs - start_ccs);
...
ccs_file = CCS_ave;
ccs_err_file = CCS_err;

fprintf(logfile,"--------------------------------------------------------\n");
fprintf(logfile,"average value of CCS = %f Ang^2\n", CCS_ave);
fprintf(logfile,"error value of CCS = %f Ang^2\n", CCS_err);
...
fprintf(logfile,"*********************************************************\n");
fprintf(logfile,"Geometric Ellipsoid: \n");
fprintf(logfile,"*********************************************************\n");
fprintf(logfile,"maximal distances: %f %f %f Ang\n", maxX, maxY, maxZ);

a = maxX + radius + skin + d;
b = maxY + radius + skin + d;
c = maxZ + radius + skin + d;

fprintf(logfile,"Initial axis length: %f %f %f Ang\n", a, b, c);
