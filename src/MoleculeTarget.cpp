...
MoleculeTarget::MoleculeTarget(string &filename, unsigned int gas_buffer_flag, string &user_ff, unsigned int user_ff_flag, unsigned int force_type, FILE* logfile) {
...
if (extension == "pqr") {
  ...
  fprintf(logfile,"only acceptable PQR or XYZ format\n");
  exit (EXIT_FAILURE);
}
...
if (user_ff_flag) fprintf(logfile,"Atom type not found in the file user force field\n");
else fprintf(logfile,"Atom type not found in the default data base\n");
exit (EXIT_FAILURE);
...
void MoleculeTarget::printFF() {
  fprintf(logfile,"*********************************************************\n");
  fprintf(logfile,"Force Field parameters: \n");
  fprintf(logfile,"*********************************************************\n");
  ...
  if (gas_buffer_flag == 3) {
    ...
    fprintf(logfile,"Symbol  mass (amu)  epislon (kcal/mol)  sigma(Angstroms) \n");
    for (int i = 0; i < nparameters; i++) {
      fprintf(logfile,"%s %s %f %f %f\n", user_atomName[i].c_str(), "-C ", user_m[i], sqrt(user_eps[i]*0.055), 0.5*(user_sig[i]+2.757));
    }
    for (int i = 0; i < nparameters; i++) {
      fprintf(logfile,"%s %s %f %f %f\n", user_atomName[i].c_str(), "-O ", user_m[i], sqrt(user_eps[i]*0.159), 0.5*(user_sig[i]+3.033));	     
    }
  } else {
    fprintf(logfile,"Symbol  mass (amu)  epislon (kcal/mol)  sigma(Angstroms) \n");
    for (int i = 0; i < nparameters; i++) {
      fprintf(logfile,"%s %f %f %f\n", user_atomName[i].c_str(), user_m[i], user_eps[i], user_sig[i]);
    }
  }
}
...
fprintf(logfile,"*********************************************************\n");
fprintf(logfile,"MOLECULE:: orientation around the inertia principal axis \n");
fprintf(logfile,"*********************************************************\n");
fprintf(logfile,"Inertia matrix a.u. Ang^2: \n");
fprintf(logfile,"{  %f  %f  %f  }\n", inertia[0][0], inertia[0][1], inertia[0][2]);
fprintf(logfile,"{  %f  %f  %f  }\n", inertia[1][0], inertia[1][1], inertia[1][2]);
fprintf(logfile,"{  %f  %f  %f  }\n", inertia[2][0], inertia[2][1], inertia[2][2]);
if (diagonal == false) {
fprintf(logfile,"Rotation matrix applied to molecule: \n");
fprintf(logfile,"{  %f  %f  %f  }\n", inertiaVectors[0][0], inertiaVectors[0][1], inertiaVectors[0][2]);
fprintf(logfile,"{  %f  %f  %f  }\n", inertiaVectors[1][0], inertiaVectors[1][1], inertiaVectors[1][2]);
fprintf(logfile,"{  %f  %f  %f  }\n", inertiaVectors[2][0], inertiaVectors[2][1], inertiaVectors[2][2]);
}
fprintf(logfile,"Molecule radius: %f Ang\n", moleculeRadius);
}
